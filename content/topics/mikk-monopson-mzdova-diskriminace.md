---
title: "Monopson a mzdová diskriminace"
course: mikk
type: topic
tags: [mikk, mikroekonomie, monopson, mzdova-diskriminace, trh-prace]
...
> [!tip] TL;DR
> **Monopson** je trh s jediným nakupujícím — zrcadlový obraz monopolu. Monopsonista platí cenu (typicky mzdu) **nižší než mezní náklady** na další jednotku vstupu, protože při najmutí dodatečného pracovníka musí zvýšit mzdu *všem* dosavadním. Sílu monopsonu určují tři faktory: elasticita tržní nabídky, počet nakupujících a míra jejich vzájemné interakce. **Mzdová diskriminace** je analogií cenové diskriminace přenesenou na trh vstupů — monopsonista se snaží odčerpat ekonomickou rentu pracovníka tím, že každému (či každé skupině) platí tolik, kolik je ochoten/ochotna přijmout. Politickou protisilou je minimální mzda a kolektivní vyjednávání odborů.

Tato stránka shrnuje látku [[mikk|Mikroekonomie 2 (mikK)]] věnovanou nedokonalé konkurenci na straně poptávky po vstupech a paralelu k [[mikk-monopol-pokrocily|monopolu]] a [[mikk-cenova-diskriminace|cenové diskriminaci]].
...
> [!note] Klíčové pojmy
> - **Monopson** — trh s pouze jediným nakupujícím (typicky jediný zaměstnavatel pro daný typ práce v daném regionu, nebo jediný odběratel pro určitý vstup).
> - **Oligopson** — trh s několika málo nakupujícími. Tržní strukturu typicky vidíme v automobilovém průmyslu (oligopson velkých automobilek vůči stovkám menších dodavatelů dílů) nebo v retailu.
> - **Monopsoní síla** — schopnost nakupujícího ovlivnit tržní cenu vstupu a zaplatit *nižší cenu, než jaká by odpovídala dokonale konkurenčnímu trhu*. Operativně: schopnost platit méně než hodnotu mezního produktu vstupu, případně méně než mezní náklady na další jednotku vstupu.

Monopson tedy stojí naproti [[mikk-monopol-pokrocily|monopolu]]: zatímco monopolista je jediným prodávajícím a využívá své postavení k tomu, aby účtoval cenu *nad* MC, monopsonista je jediným nakupujícím a využívá svého postavení k tomu, aby platil cenu *pod* svým mezním ekonomickým přínosem (MRP, *marginal revenue product*). Obě struktury jsou symetrickými odchylkami od dokonalé konkurence a obě generují *deadweight loss* — viz dále.
...
> [!example] Anekdota: *company script*
> V uhelných důlních městech 19. století dostávali horníci část mzdy ve formě *company script* — žetonů použitelných jen v obchodě patřícím firmě. Kombinace monopsonu na trhu práce a monopolu v lokálním retailu odčerpala z pracovníků prakticky veškerou rentu. Tento jev pomohl politicky prosadit pracovní legislativu v meziválečném období (Fair Labor Standards Act 1938, ustanovení o povinné výplatě zákonných peněz).

---

## 3. Geometrická analýza monopsonu

![[mikk-monopson-trh-prace.jpeg|Monopson na trhu práce — MFC nad S_L, optimum MRP_L=MFC, mzda W_M pod konkurenční W_C s exploitační oblastí]]

Monopson je zrcadlovým obrazem [[mikk-monopol-pokrocily|monopolu]]. Geometrie:

**Křivka tržní nabídky práce $S_L(W)$** má v běžném modelu pozitivní sklon — při vyšší mzdě je ochotno pracovat více lidí (vyšší mezní pracovník má vyšší rezervační mzdu, *transferovou cenu práce*).

**Mezní náklady na práci (Marginal Cost of Labor, $MCL$)** jsou *nad* křivkou $S_L$. Důvod: monopsonista platí všem zaměstnaným *jednu jednotnou mzdu*. Pokud chce najmout další jednotku práce, musí zvýšit mzdu — a to nejen pro nového pracovníka, ale **pro všechny dosud zaměstnané**. Algebraicky:

$$
TCL(L) = W(L) \cdot L \quad \Rightarrow \quad MCL(L) = \frac{dTCL}{dL} = W(L) + L \cdot \frac{dW}{dL}
$$

Druhý člen $L \cdot \frac{dW}{dL}$ je kladný (rostoucí $S_L$), takže $MCL > W$ pro každé $L > 0$.

**Křivka poptávky po práci** je dána hodnotou mezního produktu — *Marginal Revenue Product of Labor*:

$$
MRPL(L) = MR \cdot MPL(L)
$$

(Pro firmu na dokonale konkurenčním trhu výstupu se zjednoduší na $VMPL = P \cdot MPL$.)

**Optimum monopsonu** stanoví firma stejně, jako každý optimalizující subjekt — tam, kde se mezní přínos rovná mezním nákladům:

$$
\boxed{MCL(L^\ast) = MRPL(L^\ast)}
$$

**Mzda** se ovšem stanoví jinak: monopsonista zaplatí pracovníkům přesně tolik, kolik stačí k tomu, aby jich přišlo $L^\ast$. To čte z křivky nabídky:

$$
W^\ast = S_L^{-1}(L^\ast) < MRPL(L^\ast)
$$

Klíčový důsledek: **mzda v monopsonu je nižší než hodnota mezního produktu práce**. Rozdíl $MRPL(L^\ast) - W^\ast$ je *míra monopsoního vykořisťování* (Pigou) — část toho, co pracovník přidává firmě k tržbě, si firma ponechává jako monopsoní rentu.

> [!note] Srovnání s dokonalou konkurencí
> Na dokonale konkurenčním trhu práce platí $W = MRPL$ — firma je *price-taker* a najímá tolik práce, dokud poslední pracovník stále přidává více, než kolik stojí. Monopsonista najímá *méně* práce ($L^\ast < L^{DK}$) a platí *nižší* mzdu ($W^\ast < W^{DK}$). Vzniká **deadweight loss** — část přebytku, který by ve volné soutěži připadl pracovníkům.

**Geometricky** se monopsoní renta zobrazuje jako obdélník mezi křivkou nabídky práce a hodnotou MRPL na intervalu zaměstnaných pracovníků $[0, L^\ast]$, redukovaný o ztrátu z neefektivně nezaměstnaných pracovníků mezi $L^\ast$ a $L^{DK}$.

### Algebraický příklad

Předpokládejme lineární nabídku práce $W(L) = 10 + 2L$ a lineární MRPL $= 50 - 3L$. Pak:

- Celkové náklady na práci: $TCL(L) = W(L) \cdot L = (10 + 2L) \cdot L = 10L + 2L^2$.
- Mezní náklady na práci: $MCL(L) = \frac{dTCL}{dL} = 10 + 4L$.
- Optimum monopsonu: $10 + 4L = 50 - 3L \Rightarrow 7L = 40 \Rightarrow L^\ast \approx 5{,}71$.
- Mzda monopsonisty: $W^\ast = 10 + 2 \cdot 5{,}71 \approx 21{,}43$.
- Pro srovnání DK: $W = MRPL \Rightarrow 10 + 2L = 50 - 3L \Rightarrow L^{DK} = 8$, $W^{DK} = 26$.
...
Stupeň monopsoní síly závisí na **třech podobných faktorech** jako monopolní síla — symetrie je dokonalá (paralela viz [[mikk-monopol-pokrocily|Lerner index pro monopol]]):
...
$$
L_M = \frac{MRPL - W}{W} = \frac{1}{\varepsilon_S}
$$

kde $\varepsilon_S$ je cenová elasticita nabídky práce. Monopsoní marže je tedy přímo dána inverzí elasticity — stejný princip jako u monopolu, kde marže $\frac{P-MC}{P} = -\frac{1}{\varepsilon_D}$. Pro $\varepsilon_S = 1$ je marže 100 % (mzda dvakrát menší než $MRPL$); pro $\varepsilon_S \to \infty$ jde marže k 0 (dokonalá konkurence).
...
> [!warning] Symetrie s monopolem
> Tyto tři faktory jsou *přesnou* symetrií tří faktorů monopolní síly — elasticita poptávky, počet prodávajících, interakce mezi prodávajícími. Tahle symetrie je didakticky důležitá: pochopení monopolu (viz [[mikk-monopol-pokrocily]]) okamžitě dává pochopení monopsonu, jen se vymění pohled "z druhé strany trhu".
...
$$
\text{Monopsoní renta} = L^\ast \cdot \big(MRPL(L^\ast) - W^\ast\big)
$$
...
> [!important] Etika a regulace
> Cenová diskriminace v produktech (např. studentské slevy, slevy seniorům) je obvykle akceptována. Mzdová diskriminace narazí na hranice rovnosti před zákonem mnohem dříve, právě proto, že se týká rozdělení důchodu, ne jen distribuce spotřeby.
...
- **Méně zaměstnanosti** ($L^\ast < L^{DK}$) → vyšší nedobrovolná nezaměstnanost, vyšší tlak na sociální systém.
...
Důležitý makroekonomický rozměr: pokud je monopsoní síla rozšířená napříč ekonomikou, klesá **podíl práce na národním důchodu** ($\frac{wL}{Y}$). Tento ukazatel v USA i evropských ekonomikách od 80. let postupně klesal — z přibližně 65 % na 58 % HDP. Část literatury vysvětluje pokles růstem koncentrace v sektorech (Autor et al. 2020, *The Fall of the Labor Share and the Rise of Superstar Firms*); tento vývoj je konsistentní s rostoucí monopsoní silou velkých firem.
...
> [!note] Cíle odborů — klasická triáda
> **A) Maximalizace ekonomické renty členů odborů.** Cílem je maximalizovat součin $(W - W_R) \cdot L$, kde $W_R$ je *transferová cena práce* (rezervační mzda — minimum, za které je daná jednotka práce ochotna vstoupit na trh práce). Geometricky: bod, kde mezní výnos z dalšího člena odborů je roven jeho transferové ceně. Tento cíl odpovídá maximalizaci čistého přebytku pracovníků.
>
> **B) Maximalizace objemu mezd (wage bill) členů odborů.** Cíl je maximalizovat $W \cdot L$ — celkovou výplatní listinu. Z vlastnosti vztahu mezi celkovou a mezní veličinou plyne, že tento výraz je maximální tam, kde **mezní mzda = 0**, tj. v bodě jednotkové elasticity poptávky po práci.
>
...
> [!important] Klíčový teoretický bod
> V tradičním modelu dokonalé konkurence vede minimální mzda nad rovnovážnou úroveň ($W_{min} > W^{DK}$) ke vzniku nezaměstnanosti — firmy najímají méně, než kolik lidí je ochotno za $W_{min}$ pracovat.
>
> **V monopsonu je situace odlišná.** Pokud je $W_{min}$ stanoveno mezi $W^\ast$ (monopsoní mzda) a $MRPL(L^\ast)$ (hodnota mezního produktu), pak:
> 1. Monopsonista už nemá motivaci platit méně — zákonný strop ho nutí platit minimálně $W_{min}$.
> 2. Mezní náklady na práci se na intervalu pod $W_{min}$ stanou *konstantními* a rovnými $W_{min}$ (firma platí všem stejně, ale nyní *zákonem* danou výši).
...
> [!warning] Hranice argumentu
> Argument neplatí univerzálně. Pokud je $W_{min}$ stanovena *nad* $MRPL(L^\ast)$, zaměstnanost klesne i v monopsonu — model dokonalé konkurence se znovu prosadí. Empirické debaty (Neumark, Wascher vs. Card, Krueger) se nevedou o teoretické možnosti, ale o tom, kde reálně leží $W_{min}$ vůči $MRPL$ v konkrétních sektorech.

---

## 11. Vztah ke monopolu a další souvislosti

Monopson a [[mikk-monopol-pokrocily|monopol]] jsou strukturálně symetrické — oba generují deadweight loss, oba motivují k regulačnímu zásahu, oba se mohou stupňovat cenovou/mzdovou diskriminací. Učebnicové srovnání:

| | Monopol | Monopson |
|---|---|---|
| pozice | jediný prodávající | jediný kupující |
| ovlivňuje | cenu výstupu | cenu vstupu |
| optimum | $MR = MC$ | $MRP = MCL$ |
| cena vs. mezní hodnota | $P > MC$ | $W < MRPL$ |
...
V praxi často firma kombinuje obojí — je [[mikk-monopol-pokrocily|monopolistou]] na trhu výstupu (např. zdravotní pojišťovna jako prodávající pojištění) i monopsonistou na trhu vstupu (jako kupec lékařských služeb). Tomu se říká **bilaterální tržní moc**.

Vztah k [[mikk-prirozeny-monopol-regulace|přirozenému monopolu]] je nepřímý: některé monopsony jsou důsledkem síťových efektů na straně poptávky (jediná železnice, jediný stát jako kupec specifické technologie), což připomíná logiku přirozeného monopolu posunutou na druhou stranu trhu.

Pro úplný obraz fungování trhu práce je třeba kombinovat monopson s analýzou [[mikk-elasticita-poptavky|elasticity poptávky]] po práci a [[mikk-cenova-diskriminace|cenové diskriminace]] na trhu výstupu — výsledná mzda totiž nezávisí jen na trhu práce, ale i na tržní struktuře, ve které firma prodává své výstupy. Pokud je firma monopolistou na trhu výstupu, je její $MRPL$ nižší (a méně elastické) než u konkurenční firmy, takže monopsoní efekt ve mzdě se prohlubuje.

---

## 12. Příklady ze zkoušek

V [[mikk-vzorove-zkousky|předtermínech]] se monopson objevuje opakovaně:

- **Otázka *labor demand under monopsony*** — odvození $MCL > W$, výpočet optimálního $L^\ast$ z $MCL = MRPL$ a srovnání s dokonalou konkurencí. Typicky se zadá lineární $S_L: W = a + bL$ a lineární $MRPL: W = c - dL$, pak $MCL = a + 2bL$ a optimum $L^\ast = (c-a)/(2b+d)$, mzda $W^\ast = a + bL^\ast$.
...
Pro řešené příklady viz [[mikk-vzorce-prehled|přehled vzorců]] a [[mikk-vzorove-zkousky|vzorové zkoušky]].

### Návod na řešení

Standardní postup u zkouškové úlohy *labor demand under monopsony*:

1. **Identifikujte tržní strukturu.** Pokud zadání obsahuje "jediný zaměstnavatel", "dominantní firma v regionu" nebo "firma čelí celé tržní nabídce", jde o monopson.
2. **Sestrojte $TCL$ a $MCL$.** Z dané křivky $W(L)$ vypočtěte $TCL = W(L) \cdot L$, derivujte podle $L$, dostanete $MCL$. Pro lineární $W = a + bL$ je $MCL = a + 2bL$ (sklon dvojnásobný oproti $W$ — analogie MR vs. P u monopolu).
3. **Stanovte podmínku optima**: $MCL = MRPL$. Vyřešte pro $L^\ast$.
4. **Mzda**: dosaďte $L^\ast$ zpět do $W(L)$, ne do $MRPL$.
5. **Renta a DWL**: vypočtěte rozdíl proti dokonalé konkurenci, kde $W = MRPL$.

Klasická chyba, na kterou opraváři čekají: studenti dosadí $L^\ast$ do $MRPL$ místo do $W(L)$. To jim dá hodnotu mezního produktu, ne reálnou mzdu — a celá pointa monopsonu se ztrácí.
...
1. **Geometrie**: $MCL > W$, optimum $MCL = MRPL$, mzda čtená z $S_L$ pod $MRPL$.
2. **Faktory síly**: nízká elasticita nabídky, malý počet kupujících, nízká interakce mezi kupujícími.
3. **Mzdová diskriminace** (1./2./3. stupně) jako rozšiřující nástroj odčerpávání renty.
4. **Empirické důkazy**: Manning (2003), Silicon Valley no-poach kauza, Card & Krueger (1994).
5. **Protisíly**: minimální mzda (paradoxně může v monopsonu *zvýšit* zaměstnanost), odbory jako bilaterální monopol s definovanými cíli (renta, wage bill, zaměstnanost).
...
> [!tip] Krátká *cheat sheet*
> 1. **Monopson = jediný kupující.** Zrcadlo monopolu.
> 2. **MCL > W** vždy (pro $L > 0$, rostoucí $S_L$). Důvod: zvýšení mzdy při najmutí dalšího se týká *všech*.
> 3. **Optimum**: $MCL = MRPL$. Mzda se čte z $S_L$ pod $MRPL$.
> 4. **Tři faktory síly**: nízká elasticita $S_L$, málo kupujících, slabá konkurence mezi kupujícími.
> 5. **Mzdová diskriminace**: 1. stupeň = perfect (rezervační mzda každého), 2. stupeň = balíčky, 3. stupeň = skupiny.
> 6. **Cíle odborů**: renta (A), wage bill (B), zaměstnanost (C). Bilaterální monopol s monopsonistou.
> 7. **Minimální mzda v monopsonu může současně zvýšit mzdu i zaměstnanost** — to je hlavní teoretický bod, který se ptá ve zkouškách.
> 8. **Klasická zkušební past**: dosadit $L^\ast$ do $MRPL$ místo do $W(L)$. Nedělej to.

---

**Související stránky**: [[mikk|Mikroekonomie 2 (mikK)]] · [[mikk-monopol-pokrocily|Monopol — pokročilá analýza]] · [[mikk-prirozeny-monopol-regulace|Přirozený monopol a regulace]] · [[mikk-cenova-diskriminace|Cenová diskriminace]] · [[mikk-elasticita-poptavky|Elasticita poptávky]] · [[mikk-vzorce-prehled|Přehled vzorců]] · [[mikk-vzorove-zkousky|Vzorové zkoušky]]
