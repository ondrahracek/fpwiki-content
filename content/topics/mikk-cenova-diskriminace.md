---
title: "Cenová diskriminace"
course: mikk
type: topic
tags: [mikk, mikroekonomie, cenova-diskriminace, monopol, kupon]
sources: [raw/mikk/mik2K prednaska 3 blok 2026.pdf, raw/mikk/Prikady pro KS 5 prednaska reseni 2026.pdf, raw/mikk/Prikady pro KS 5 2026.pdf]
created: 2026-04-27
updated: '2026-04-27'
---
...
Tato stránka rozvíjí téma započaté v [[mikk-monopol-pokrocily|monopolu]] a navazuje na [[mikk-elasticita-poptavky|elasticitu poptávky]] a [[mikk-bundling-two-part-tariff|bundling a two-part tariff]]. Je součástí kurzu [[mikk|Mikroekonomie 2 (mikK)]].
...
Monopolista, který stanoví **jednu jedinou cenu** $P^*$ na základě pravidla $MR = MC$, prodá množství $Q^*$. Při této strategii získá:

- **Plochu A** = monopolní zisk při ceně $P^*$ (obdélník mezi $P^*$ a $MC$ od $0$ do $Q^*$).

Část přebytku spotřebitele, který by zákazníci s vyšší ochotou platit byli ochotni odevzdat, **uniká firmě jako spotřebitelský přebytek** (plocha mezi poptávkou $D$ a cenou $P^*$ nad $P^*$). A další část (plocha $B$) je **deadweight loss** — produkce, která se neuskuteční, protože monopolní cena $P^*$ je nad mezními náklady, ale pod některými rezervačními cenami.
...
1. **Tržní síla (price-making power).** Firma musí mít schopnost ovlivňovat cenu — typicky monopol, monopolistická konkurence nebo oligopol. V dokonalé konkurenci firma cenu pouze přebírá a diskriminace je vyloučena.
2. **Segmentace.** Firma musí dokázat rozpoznat různé skupiny zákazníků (nebo různé situace/množství) a aplikovat pro ně jiné ceny. To je často ten technicky nejobtížnější bod.
3. **Zabránění přeprodeji (resale arbitrage).** Pokud zákazník s nízkou cenou může produkt přeprodat zákazníkovi s vysokou cenou, diskriminace zkolabuje. Proto se diskriminace lépe uplatňuje u **služeb** (ostříhání, hotel, doprava) než u převoditelného zboží.

---

## 2. Tři stupně cenové diskriminace — souhrn

![[mikk-cenova-diskriminace-3stupne.jpeg|Tři stupně cenové diskriminace — perfektní (1.), block pricing (2.) a segmentační (3.) — vedle sebe]]
...
- 3. stupeň — firma **rozdělí zákazníky podle pozorovatelné charakteristiky** (věk, ID studenta, region) a každé skupině nastaví jinou cenu, na kterou ostatní nedosáhnou.
...
Z pohledu firmy přestává platit, že existuje jediná „cena". Místo ní existuje **cenová funkce**: $P_i$ pro zákazníka $i$, rovna jeho $WTP_i$ (willingness-to-pay).

### 3.2 Geometrie a důsledek pro zisk

Při 1. stupni diskriminace se poptávková křivka $D$ stává zároveň **mezním příjmem** firmy: každá další jednotka prodaná za rezervační cenu konkrétního zákazníka přidá k tržbám přesně tu cenu (žádné ztráty z toho, že by se musela snížit cena u dříve prodaných jednotek).

$$MR_{1.\text{ stupeň}} = D = AR$$

Optimum je tam, kde:

$$D = MC$$

Tj. firma vyrábí až do bodu $Q^{**}$, kde poptávka protíná mezní náklady. To je **stejné množství jako v dokonalé konkurenci** — rozdíl je jen v rozdělení přebytku:

- V dokonalé konkurenci: vyrobí se $Q^{**}$, cena = $MC$, **veškerý přebytek jde spotřebitelům**.
- V 1. stupni diskriminace: vyrobí se $Q^{**}$, **veškerý přebytek jde firmě**.
...
1. **Spotřebitelský přebytek při ceně $P^*$** (trojúhelník nad $P^*$ a pod $D$ od $0$ do $Q^*$) — tato plocha se převádí ze spotřebitelů na firmu.
2. **Plocha mezi $P^*$ a $MC$ od $Q^*$ do $Q^{**}$** — to je dříve „zabitá" plocha (deadweight loss z monopolu), která se nyní díky rozšíření produkce realizuje a přidá k zisku.
...
> **Odpověď:**  
> 1. Je **příliš mnoho spotřebitelů** — nepraktické udržovat individuální vyjednávání s každým z nich.  
> 2. **Nelze stanovit rezervační cenu** pro každého zákazníka — tu zákazník přirozeně skrývá, protože její odhalení mu škodí.
...
Místo nekonečně mnoha cen lze použít **několik cenových hladin** ($P_1, P_2, \ldots, P_6$). Při šesti hladinách už firma získá podstatnou část spotřebitelského přebytku oproti jediné ceně. Schéma vypadá jako „schody" pod poptávkovou křivkou.
...
- **Univerzitní stipendia** v USA — „price discrimination by ability to pay": uvedená sticker-price je $70 000, ale po posouzení rodinných příjmů studentů se efektivní ceny pohybují od $5 000 do $70 000.

### 3.7 Detail z Předtermínu C — plastický chirurg

V jednom ze starších předtermínových zadání se objeví příklad **plastického chirurga**, jehož $LAC = 7500$ Kč/operace a který diskriminuje podle bonity klienta. Plný rozbor je v [[mikk-vzorove-zkousky]]; klíčové body:
...
- Optimální je obsloužit jen ty klienty, kteří nabídnou alespoň $LAC$, a od každého inkasovat maximum.
...
- **První blok** ($0$ až $Q_1$ jednotek) je dražší (cena $P_1$).
- **Druhý blok** ($Q_1$ až $Q_2$) je levnější (cena $P_2 < P_1$).
- **Třetí blok** atd.
...
Poptávková křivka má schodovitý tvar — pro každý blok jiná cena. Tržby firmy z bloku $i$ jsou $P_i \cdot (Q_i - Q_{i-1})$. Optimální počet a výška bloků se hledá tak, aby se maximalizoval součet tržeb minus náklady.
...
| **SaaS** | „$10 za prvních 1000 API volání, $5 za každé další tisíce." |
...
1. **Trh lze rozdělit na minimálně 2 skupiny** podle pozorovatelné charakteristiky (věk, příjem, region, čas, status).
2. **Každá skupina má vlastní poptávkovou křivku** — typicky se liší elasticitou.
3. **Skupiny mezi sebou nemohou obchodovat** (nemůže nastat resale).

### 5.2 Pravidlo optimalizace

Firma řeší úlohu maximalizace celkového zisku $\pi = TR_1 + TR_2 + \ldots + TR_n - TC(Q)$, kde $Q = \sum Q_i$. Z parciálních derivací:

$$MR_1 = MR_2 = \ldots = MR_n = MC$$
...
1. Sestrojte $MR_1$ a $MR_2$ pro každou skupinu.
2. Sestrojte **horizontální součet** $MR_T = MR_1 + MR_2$ (pro každou hodnotu $MR$ sečtete odpovídající množství z obou skupin).
3. Najděte $Q_T$, kde $MR_T = MC$.
4. Z odpovídající hodnoty $MR^*$ zpětně určete $Q_1$ a $Q_2$ (tj. pro jaké množství v každé skupině platí $MR_i = MR^*$).
5. Z poptávkových křivek $D_1, D_2$ určete ceny $P_1, P_2$.

### 5.4 Důsledek pro vztah cena × elasticita

Z pravidla $MR_i = MC$ a Lernerova vztahu plyne:
...
Pokud je poptávka skupiny 1 příliš nízká (resp. její $D_1$ leží zcela pod cenou, kterou firma účtuje skupině 2), pak:

$$D_1 < MC \text{ pro každé } Q_1 > 0$$

V takovém případě firma **neprodává skupině 1 vůbec** — bylo by to ztrátové. Optimální je obsluhovat **jen skupinu 2** za jednu cenu $P^*$, jako klasický monopol.

> **Geometricky:** Pokud na grafu skupina 1 leží zcela vlevo a její $D_1$ je nižší než $MC$ pro všechna $Q$, pak platí $Q_1^* = 0$.
...
## 6. Detailní příklad — diskriminace na 2 trzích, $TC = Q^2 + 10Q$

### 6.1 Zadání

Monopolista vyrábí v jednom podniku a prodává na dvou trzích. Nákladová funkce $TC = Q^2 + 10Q$, poptávkové funkce:
...
### 6.2 Metoda A — horizontální součet $MR$

**Krok 1: Inverze poptávek.** Z $Q_1 = 32 - 0{,}4 P_1$ vyjádříme $P_1$:

$$P_1 = \frac{32 - Q_1}{0{,}4} = 80 - 2{,}5 Q_1$$

Analogicky pro druhý trh:

$$P_2 = \frac{18 - Q_2}{0{,}1} = 180 - 10 Q_2$$

**Krok 2: Mezní příjmy.** Pro lineární poptávku platí, že $MR$ má stejný průsečík s osou $P$ jako $D$, ale dvojnásobnou strmost:

$$MR_1 = 80 - 5 Q_1 \quad\Rightarrow\quad Q_1 = 16 - 0{,}2\, MR_1$$
$$MR_2 = 180 - 20 Q_2 \quad\Rightarrow\quad Q_2 = 9 - 0{,}05\, MR_2$$

**Krok 3: Mezní náklady.** $TC = Q^2 + 10Q$, kde $Q = Q_1 + Q_2$, takže:

$$MC = \frac{dTC}{dQ} = 2Q + 10$$

**Krok 4: Horizontální součet.** Při optimu platí $MR_1 = MR_2 = MR$, tedy:

$$Q = Q_1 + Q_2 = (16 - 0{,}2 MR) + (9 - 0{,}05 MR) = 25 - 0{,}25 MR$$

Inverzí: $MR = \dfrac{25 - Q}{0{,}25} = 100 - 4Q$.

**Krok 5: Rovnice $MR = MC$.**

$$100 - 4Q = 2Q + 10$$
$$90 = 6Q$$
$$Q = 15$$

A tedy $MR = 100 - 4 \cdot 15 = 40$.

**Krok 6: Zpětný rozpad na trhy.**

$$Q_1 = 16 - 0{,}2 \cdot 40 = 16 - 8 = 8$$
$$Q_2 = 9 - 0{,}05 \cdot 40 = 9 - 2 = 7$$

**Krok 7: Ceny.**

$$P_1 = 80 - 2{,}5 \cdot 8 = 80 - 20 = 60$$
...
Alternativní (a robustnější — funguje pro libovolné nelineární $TC$) postup:

$$\pi = TR_1 + TR_2 - TC = P_1 Q_1 + P_2 Q_2 - (Q_1+Q_2)^2 - 10(Q_1+Q_2)$$

Po dosazení $P_i$ z inverzních poptávek:

$$\pi = (80 - 2{,}5 Q_1) Q_1 + (180 - 10 Q_2) Q_2 - (Q_1+Q_2)^2 - 10(Q_1+Q_2)$$

Roznásobím a sloučím:

$$\pi = 80 Q_1 - 2{,}5 Q_1^2 + 180 Q_2 - 10 Q_2^2 - Q_1^2 - 2 Q_1 Q_2 - Q_2^2 - 10 Q_1 - 10 Q_2$$

$$\pi = -3{,}5 Q_1^2 - 11 Q_2^2 - 2 Q_1 Q_2 + 70 Q_1 + 170 Q_2$$

Parciální derivace:

$$\frac{\partial \pi}{\partial Q_1} = -7 Q_1 - 2 Q_2 + 70 = 0$$
$$\frac{\partial \pi}{\partial Q_2} = -22 Q_2 - 2 Q_1 + 170 = 0$$

Soustava:

$$7 Q_1 + 2 Q_2 = 70 \quad (\text{I})$$
$$2 Q_1 + 22 Q_2 = 170 \quad (\text{II})$$

Z (I): $Q_2 = (70 - 7 Q_1) / 2 = 35 - 3{,}5 Q_1$. Dosadím do (II):

$$2 Q_1 + 22 (35 - 3{,}5 Q_1) = 170$$
$$2 Q_1 + 770 - 77 Q_1 = 170$$
$$-75 Q_1 = -600$$
$$Q_1 = 8$$

A z (I): $Q_2 = (70 - 56)/2 = 7$. ✓ **Shoda s metodou A.**

### 6.4 Diskuze realizovatelnosti

Aby tato cenová politika fungovala:

- **Trhy musí být oddělené** (geograficky nebo institucionálně) — zákazník platící $60 nesmí přeprodávat zákazníkovi platícímu $110.
...
## 7. Detailní příklad — konstantní $MC$, dva oddělené trhy

### 7.1 Zadání

Monopolista vyrábí s konstantními náklady $5$ Kč/jednotku, prodává na dvou oddělených trzích:

$$Q_1 = 55 - P_1 \quad\Leftrightarrow\quad P_1 = 55 - Q_1$$
$$Q_2 = 70 - 2 P_2 \quad\Leftrightarrow\quad P_2 = 35 - 0{,}5 Q_2$$

### 7.2 Řešení s diskriminací

Při konstantních $MC$ jsou trhy v podstatě **nezávislé** — můžu řešit každý trh zvlášť.

**Trh 1:** $MR_1 = 55 - 2 Q_1 = MC = 5$. Tedy:

$$2 Q_1 = 50 \Rightarrow Q_1 = 25, \quad P_1 = 55 - 25 = 30$$

**Trh 2:** $MR_2 = 35 - Q_2 = MC = 5$. Tedy:

$$Q_2 = 30, \quad P_2 = 35 - 0{,}5 \cdot 30 = 20$$

**Celková výroba:** $Q = 25 + 30 = 55$.

**Zisky:**

$$\pi_1 = (P_1 - MC) Q_1 = (30 - 5) \cdot 25 = 25 \cdot 25 = 625$$
$$\pi_2 = (P_2 - MC) Q_2 = (20 - 5) \cdot 30 = 15 \cdot 30 = 450$$
$$\pi_{\text{disk}} = 625 + 450 = 1075$$

**Kontrola horizontálním součtem.** $Q_1 = 27{,}5 - 0{,}5 MR_1$, $Q_2 = 35 - MR_2$. Při $MR_1 = MR_2 = MR$:

$$Q = 62{,}5 - 1{,}5 MR \Rightarrow MR = 41{,}\overline{6} - 0{,}\overline{6} Q$$

$MR = MC$: $41{,}\overline{6} - 0{,}\overline{6} Q = 5 \Rightarrow Q = 55$ ✓.

> **Pozor:** Při konstantních $MC$ vyjde celkové množství stejné jako bez diskriminace. **Při rozdílných (rostoucích či klesajících) $MC$ by tomu tak nebylo** — diskriminace by přerozdělila množství mezi trhy a změnila i celkový objem produkce.

### 7.3 Řešení bez diskriminace (jednotná cena)

Sjednocená poptávka: $Q = Q_1 + Q_2 = (55 - P) + (70 - 2P) = 125 - 3P$.

Inverzí: $P = \dfrac{125 - Q}{3} = 41{,}\overline{6} - 0{,}\overline{3} Q$.

$MR = 41{,}\overline{6} - 0{,}\overline{6} Q = MC = 5 \Rightarrow Q = 55$, $P = 41{,}\overline{6} - 0{,}\overline{3} \cdot 55 = 41{,}\overline{6} - 18{,}\overline{3} = 23{,}\overline{3}$.

Zisk: $\pi_{\text{nedisk}} = (P - MC) \cdot Q = (23{,}\overline{3} - 5) \cdot 55 = 18{,}\overline{3} \cdot 55 = 990$.

### 7.4 Benefit z diskriminace

$$\Delta \pi = \pi_{\text{disk}} - \pi_{\text{nedisk}} = 1075 - 990 = 85$$
...
| Produkt | Neuživatelé kuponů $E_1$ | Uživatelé kuponů $E_2$ |
|---------|---------------------------|--------------------------|
| Toaletní potřeby | $-0{,}60$ | $-0{,}66$ |
| Oblečení | $-0{,}71$ | $-0{,}96$ |
| Šampóny | $-0{,}84$ | $-1{,}04$ |
| Vaření (potřeby) | $-1{,}22$ | $-1{,}32$ |
| Polotovar pizza | $-0{,}88$ | $-1{,}09$ |
| Koláče | $-0{,}21$ | $-0{,}43$ |
...
Ze vztahu $MR_i = MC$ a Lernerova pravidla pro 3. stupeň diskriminace:

$$\frac{P_1}{P_2} = \frac{1 + 1/E_2}{1 + 1/E_1}$$

kde $P_1$ je cena pro neuživatele (skupina 1) a $P_2$ je cena s kuponem (skupina 2). Elasticity berte se znaménkem.

### 8.4 Konkrétní výpočet — koláče

Pro koláče: $E_1 = -0{,}21$ (občasní), $E_2 = -0{,}43$ (uživatelé kuponů).

$$\frac{P_1}{P_2} = \frac{1 + 1/(-0{,}43)}{1 + 1/(-0{,}21)} = \frac{1 - 2{,}326}{1 - 4{,}762} = \frac{-1{,}326}{-3{,}762} \approx 0{,}352$$

Pozn.: tento vzorec předpokládá, že obě elasticity jsou v absolutní hodnotě > 1 (jinak je $MR$ záporný a optimalizace nemá smysl). Pro koláče v této tabulce není přísně splněno — proto je správnější interpretace: **firma dokáže účtovat neuživatelům cenu zhruba 1{,}67× vyšší než uživatelům** (tj. plná cena : kupónová cena ≈ 1{,}67 : 1).

> **Číselný příklad:** Koláč prodáván za $1{,}50$ — pak s kuponem stojí přibližně **90 centů**. ($1{,}50 / 1{,}67 \approx 0{,}90$.)
...
- $D_1, MR_1$ — neelastická poptávka early adopters (vlevo na grafu).
- $D_2, MR_2$ — elastická poptávka mainstreamu (vpravo, položnější).

Firma nejprve operuje na trhu 1 s vysokou cenou $P_1$ a nízkým $Q_1$. Po čase přesune cenu na $P_2 < P_1$ a získá další objem $Q_2$.
...
Firma ve **špičce** (vysoká poptávka $D_1$) účtuje **vyšší cenu $P_1$**; **mimo špičku** (nízká poptávka $D_2$) účtuje **nižší cenu $P_2$**.

### 10.2 Geometrický pohled

Pro každou periodu zvlášť firma řeší $MR_i = MC$. Ve špičce má vyšší poptávku, takže optimální cena je vyšší. Mimo špičku optimální cena nižší. Mezní náklady přitom mohou být v obou periodách stejné (pokud jde o stejnou výrobu) nebo různé (např. mimo špičku se používá levnější zdroj).

### 10.3 Rozdíl od 3. stupně diskriminace

**Pozor:** Peak-load pricing **není pravá 3. stupeň diskriminace** ze dvou důvodů:

- **Mezní náklad je různý** mezi špičkou a mimošpičkou (ve špičce se přibližujeme kapacitnímu stropu → roste $MC$).
...
Tento příklad je **identický s příkladem v sekci 6** této stránky: $TC = Q^2 + 10Q$, dvě poptávky $Q_1 = 32 - 0{,}4 P_1$, $Q_2 = 18 - 0{,}1 P_2$, hledejte ceny a množství.

**Klíčové výsledky:** $Q_1 = 8$, $Q_2 = 7$, $P_1 = 60$, $P_2 = 110$, $Q = 15$, $MR^* = 40$.

Plné řešení viz sekce 6 výše.

### 13.2 Předtermín C — plastický chirurg

Plastický chirurg s $LAC = 7500$ Kč. Dva typy klientů: bohatí (vysoká rezervační cena) a střední (nižší rezervační cena). Kapacita omezená.

**Stupeň diskriminace:** 1. stupeň (individuální cena pro každého klienta na základě vyjednávání).

**Strategie:** Obsloužit klienty v pořadí podle nabídnuté ceny, dokud nabídka neklesne pod $LAC = 7500$. Každému klientovi účtovat jeho individuální maximum, nikoli jednu cenu.
...
|--------|------------------------|------|
| 1. | $D = MC$ | Individuální = $WTP_i$ |
| 2. | $MR_i = MC$ pro každý blok $i$ | Klesající podle bloku |
| 3. | $MR_1 = MR_2 = \ldots = MC$ | $\dfrac{P_1}{P_2} = \dfrac{1 + 1/E_2}{1 + 1/E_1}$ |

| Veličina | Vzorec |
|----------|--------|
| Lernerův index | $L = \dfrac{P - MC}{P} = -\dfrac{1}{E}$ |
| Mezní příjem (lineární $D$) | $MR = a - 2bQ$, pokud $D: P = a - bQ$ |
| Horizontální součet | $Q_T = \sum Q_i$ při společné hodnotě $MR$ |
...
- [[mikk-monopol-pokrocily|Monopol — pokročilá teorie]] — nutný předpoklad, cenová diskriminace je vlastně **rozšíření monopolního chování**.
- [[mikk-elasticita-poptavky|Elasticita poptávky]] — určuje, kdo je „neelastický" (platí víc) a kdo „elastický" (platí míň).
- [[mikk-bundling-two-part-tariff|Bundling a two-part tariff]] — další pokročilé cenové strategie navazující na diskriminaci.
- [[mikk-monopolisticka-konkurence|Monopolistická konkurence]] — cenová diskriminace funguje slaběji, ale stále existuje (značky a věrnost).
- [[mikk-vzorce-prehled|Přehled vzorců]] — všechny relevantní rovnice.
- [[mikk-vzorove-zkousky|Vzorové zkoušky]] — řešené příklady včetně příkladu o plastickém chirurgovi.
- [[mikk|Mikroekonomie 2 (mikK)]] — rozcestník kurzu.

---

## 16. Klíčové takeaways

1. **Cenová diskriminace = nestejné ceny stejného produktu**, motivované extrakcí spotřebitelského přebytku.
2. **3 stupně:** 1. (individuální), 2. (podle množství), 3. (podle skupin).
3. **Nutné podmínky:** tržní síla + segmentace + zabránění arbitráži.
4. **Pravidlo 3. stupně:** $MR_1 = MR_2 = \ldots = MC$. Méně elastická skupina platí víc.
5. **Kupony, časová diskriminace a peak-load pricing** jsou všechno aplikace 2. nebo 3. stupně.
6. **Diskriminace zvyšuje zisk firmy** (přínos lze přesně spočítat — viz příklad v sekci 7: $\Delta\pi = 85$ Kč).
7. **Diskriminace často zvyšuje i celkové vyrobené množství** a tím snižuje deadweight loss — paradoxně může být **alokačně efektivnější** než jednotná monopolní cena.
8. **V praxi se diskriminace maskuje** jako slevy, akce, kupony, balíčky a věrnostní programy, protože transparentní diskriminace naráží na psychologické bariéry.

---

## 17. Doplňková teorie — proč 3. stupeň vychází z $MR_1 = MR_2$

Krátká derivace optimality 3. stupně. Firma maximalizuje:

$$\pi(Q_1, Q_2) = TR_1(Q_1) + TR_2(Q_2) - TC(Q_1 + Q_2)$$

Parciální derivace podle $Q_1$:

$$\frac{\partial \pi}{\partial Q_1} = \frac{dTR_1}{dQ_1} - \frac{dTC}{dQ}\cdot \frac{\partial Q}{\partial Q_1} = MR_1 - MC = 0$$

Analogicky pro $Q_2$: $MR_2 - MC = 0$. Z toho přímo $MR_1 = MR_2 = MC$.

**Intuice:** Kdyby $MR_1 > MR_2$, firma by získala přesunutím jednotky ze trhu 2 na trh 1 dodatečný zisk $MR_1 - MR_2 > 0$ při nezměněných nákladech. Optimum je tam, kde žádný takový přesun už není výhodný — tedy při vyrovnaných mezních příjmech.

### 17.1 Vztah $MR$ a elasticity

Z definice $TR = P \cdot Q$ a derivačního pravidla:

$$MR = \frac{dTR}{dQ} = P + Q\frac{dP}{dQ} = P\left(1 + \frac{Q}{P}\frac{dP}{dQ}\right) = P\left(1 + \frac{1}{E}\right)$$

Z optimality $MR_1 = MR_2$:
...
Pokud $|E_1| < |E_2|$ (skupina 1 je méně elastická), pak $1/|E_1| > 1/|E_2|$ a $P_1 > P_2$. **Méně elastická skupina platí vyšší cenu** — to je centrální takeaway pro celou kapitolu.
